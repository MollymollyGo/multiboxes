...
async def game_loop():
    while True:
        ...
        player_positions = {player_id: (body.position.x, body.position.y, body.angle) for player_id, body in
                            players.items()}
        ...
        for client in clients:
            for player_id, position in player_positions.items():
                asyncio.get_event_loop().run_in_executor(None, client.send_message, "/update_position",
                                                         [player_id, *position])
